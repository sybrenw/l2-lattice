...
namespace Lattice.LoginServer.Network.LoginPacket.Client
{
    internal class C_0x00_RequestLogin : ReceivablePacket<LoginClient>
    {
        ...
        protected override async Task ReadAsync(BinaryReader reader)
        {
            ...
            {
                await Client.SendPacketAsync(new S_0x01_LoginFail());
                return;
            }
            ...
            if (accountId > 0)
            {
                ...
                await Client.SendPacketAsync(new S_0x03_LoginOk());
            }
            else if (accountId == (int)LoginResult.Banned)
            {
                // Account is banned
                await Client.SendPacketAsync(new S_0x02_AccountBanned(0x20));
            }
            else
            {
                // Login failed
