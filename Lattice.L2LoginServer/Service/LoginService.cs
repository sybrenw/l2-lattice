...
using Lattice.LoginServer.Database;
...
namespace Lattice.LoginServer.Service
{
    ...
    {
        private static ILogger Logger { get; } = Logging.CreateLogger<LoginService>();
        ...
        public async Task<int> Login(LoginClient client, string username, string password)
        {
            ...
            if (account != null && _sessions.TryAdd(account.AccountId, client.Session))
            {
